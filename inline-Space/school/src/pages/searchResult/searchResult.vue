<template>
	<div class="container CnBtm" :catchtouchmove="catchMove">
		...
		<div class="condiTion">
			<div class="seaInput">
				...
				<form>
					<input  type="text" placeholder="搜索商品" placeholder-class='seaPlace' confirm-type='search' />
				</form>
			</div>
			<div class="selType">
		...
		<div class="cateMenu div_float" v-show="showCateMenu">
			<div class="cateSub">
				<scroll-view class="L" scroll-y>
					<div v-for='(item,oIndex) in cateMenu' :key="oIndex" :class="[cateL == oIndex ? 'sel':'']" @click="selCateL" :data-cate-l='oIndex'>{{item.til}}</div>
				</scroll-view>
				<scroll-view class="R" scroll-y>
					<div class="cateR" v-for="(item,tIndex) in cateMenu" :key="item.id" v-show="cateL == tIndex">
						<div v-for="(item2,tIndex2) in cateMenu[tIndex].list"  :key="tIndex2"  :class="[cateL == tIndex && cateR == item2.id ? 'sel':'']" @click="selCateR" :data-cate-r='item2.id' data-choose='cate'>{{item2.name}}</div>
					</div>
				</scroll-view>
			</div>
		</div>
		...
		<div class="selCn" v-show="showQcMenu">
			<div class="selSub">
				<div class="cnList" >
					<div v-for="(item,index) in qcMenuList" :class="[chooseTwo == item.id ?'sel':'']" :key='index'  data-choose='qc' :data-c-id='item.id' @click="selCondition">{{item.til}}</div>
				</div>
			</div>
		</div>
		...
		<div class="selCn" v-show="showJlMenu">
			<div class="selSub">
				<div class="cnList">
					<div v-for="(item,index) in jlMenuList" :class="[chooseThird == item.id ?'sel':'']" :key='index'  data-choose='jl' :data-c-id='item.id' @click="selCondition">{{item.til}}</div>
				</div>
			</div>
		</div>
