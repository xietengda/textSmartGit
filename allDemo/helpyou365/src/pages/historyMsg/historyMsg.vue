	...
	export default {
		...
		methods: {
			...
			async collectPost(e){
				...
				that.Request.collectPost(wx.getStorageSync('openid'),2,postId)
					.then(res =>{
						if(res.code == 200){
							if(sign){
								wx.showToast({
			...
			async postDianz(e){
				...
				that.Request.likePost(postId,1)
					.then(res =>{
						console.log(res)
						if(res.code == 200){
							if(sign){
			...
			againFun(){
				this.historymerChant().then(res =>{wx.hideLoading()});
				this.historyPost().then(res =>{wx.hideLoading()});
			},
			//获取浏览商家列表
			async historymerChant(){
				//加载动画
				wx.showLoading({title:this.langua.loadText+'...'});
				var that = this;
				await that.Request.historymerChant(that.comPage,10,wx.getStorageSync('sessionkey'),wx.getStorageSync('language'))
					.then(res =>{
						console.log(res)
						if(res.code == 200){
			...
			async historyPost(){
				...
				wx.showLoading({title:this.langua.loadText+'...'});
				var that = this;
				await that.Request.historyPost(that.postPage,10,wx.getStorageSync('sessionkey'),wx.getStorageSync('language'))
					.then(res =>{
						console.log(res)
						if(res.code == 200){
