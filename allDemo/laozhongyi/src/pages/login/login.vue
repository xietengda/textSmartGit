		...
		<div class="accredit">
			...
			<div class="viewTwo" v-else>
				<div>
					...
					<input type="number" v-model='userPhone' placeholder="手机号码" placeholder-class='placeClass' />
				</div>
				<div>
					<span class="message"></span>
					<input type="number" v-model="userCode" placeholder="验证码" placeholder-class='placeClass' />
					<div class="send">
						<div class="sendBtn" v-if="sendMsg" @click="getuserCode">获取验证码</div>
						<div class="time" v-else>{{residueTime}}s</div>
					</div>
				</div>
				<button open-type="getUserInfo" @getuserinfo="loginFun">马上登录</button>
				<div class="back" @click="showPhone">返回</div>
			</div>
		</div>

		<div class="loginView" v-show="loginTwo">
			<div class="sub">
				<div class="headLogo logoIcon"></div>
				<div class="til">是否以授权手机号码登录</div>
		...
		methods: {
			...
			getPhoneNumber(e) {
				...
				if(e.target.encryptedData) {
					...
					//一定要在wx.login后调用wx.getUserInfo,这样才不会报错
					wx.login({
						success(lgRes) {
							//显示二次登陆
							that.loginTwo = true;

							that.Request.xcxLogin_phone(lgRes.code, encodeURIComponent(e.target.encryptedData), encodeURIComponent(e.target.iv))
								.then(res => {
									console.log(res)
									if(res.code == 200) {
										if(parseInt(res.data.countryCodec) == 86) {
											that.userPhone = res.data.purePhoneNumber;
										} else {
											that.userPhone = res.data.phoneNumber;
										}
									}
								})
					...
					success(lgRes) {
						wx.getUserInfo({
							success(wxRes) {
								...
								that.Request.login(lgRes.code, encodeURIComponent(wxRes.encryptedData), encodeURIComponent(wxRes.iv), that.userPhone)
									.then(res => {
			...
			getuserCode() {
				...
				console.log(that.util.isMobile(that.userPhone))

				if(that.userPhone == '' || !that.util.isMobile(that.userPhone)) {
					wx.showToast({
						title: '请输入正确的手机号码',
						icon: 'none'
					})
				...
				that.Request.getCode(that.userPhone)
					.then(res => {
						console.log(res)
						if(res.code == 200) {
							wx.showToast({
			...
			loginFun(user) {
				...
				that.Request.codeLogin(that.userPhone, that.userCode)
					.then(res => {
						console.log(res)
						if(res.code == 200) {
							wx.login({
								success(lgRes) {
									...
									wx.setStorageSync('userMsg', user.mp.detail.userInfo);
									wx.setStorageSync('userId', res.data.user_id);
